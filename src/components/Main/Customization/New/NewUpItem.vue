<template>
  ...
  <div class="new-up-item" v-if="dataWeek && select=='less'" >
    ...
    <div class="new-up-item-month" v-if="dataMonth&&select=='more'||area!='ALL'" >
      ...
      >
        <el-col v-for="j in i" :span="5"  :key="i.id"><PlayAlum :src="j.picUrl" :details="j.name" :id="j.id"  ></PlayAlum></el-col>
      </el-row>
    </div>
</div>
</template>
...
export default {
  ...
  computed:{
    dataMonth(){
      return this.$store.state.NewUpList[this.area]["month"]
    },
    dataWeek(){
      return this.$store.state.NewUpList[this.area]["week"]
    }
